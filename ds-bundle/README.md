...
|---|---|---|
| `Button` | `window.PhotonDS.Button` | 4 variants, 3 sizes, renders as `button` or `a` |
| `Badge` | `window.PhotonDS.Badge` | Mono chip: `tech` (grey) or `accent` (azure) |
| `Card` | `window.PhotonDS.Card` | White surface: default, `ruled`, `interactive` |
| `IconBadge` | `window.PhotonDS.IconBadge` | 44×44 icon chip: accent or `navy` |
| `Stat` | `window.PhotonDS.Stat` | Metric + suffix + mono label |
| `Eyebrow` | `window.PhotonDS.Eyebrow` | Section label, with `onInverse` for dark backgrounds |
| `Container` | `window.PhotonDS.Container` | Centred column wrapper |
| `BlueprintSection` | `window.PhotonDS.BlueprintSection` | Dark or light grid-pattern `<section>` |
| `Field` | `window.PhotonDS.Field` | Wraps label + input |
| `Input` | `window.PhotonDS.Input` | Bordered text input |
| `Textarea` | `window.PhotonDS.Textarea` | Resizable textarea |
| `Label` | `window.PhotonDS.Label` | Form label |
    ...
    <>
      ...
      <section style={{ padding: 'var(--section-y) 0', background: 'var(--bg-page)' }}>
        <Container>
          ...
          <h2 style={{ fontSize: 'var(--text-h2-size)', margin: '8px 0 40px', color: 'var(--text-strong)' }}>
            What We Deliver
          </h2>
          <div className="nb-grid-3">
            <Card>
              <h3 style={{ fontSize: 'var(--text-h3-size)', marginBottom: 8 }}>Cloud Migration</h3>
              <p style={{ color: 'var(--text-body)', fontSize: 'var(--text-sm-size)' }}>Zero-downtime migrations to AWS.</p>
            </Card>
          </div>
        </Container>
      </section>
